...
const UserFollowers: React.FC<{ userId: string }> = ({ userId }) => {
  ...
  async function fetchFollowers() {
    ...
    const { data: rels } = await supabase
      ...
      .select('user_id')
      .eq('target_id', userId);
    const followerIds = rels ? rels.map((r: any) => r.user_id) : [];
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#261531', color: '#fff', padding: 0 }}>
      <div style={{ background: '#261531', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
        ...
        <span style={{ color: '#fff', fontWeight: 700, fontSize: 26 }}>Followers</span>
      </div>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 0 0 0' }}>
        {loading ? (
          <div style={{ color: '#fff', textAlign: 'center', marginTop: 40 }}>Loading...</div>
        ) : followers.length === 0 ? (
          <div style={{ color: '#fff', opacity: 0.7, textAlign: 'center', marginTop: 40 }}>No followers yet</div>
        ) : (
